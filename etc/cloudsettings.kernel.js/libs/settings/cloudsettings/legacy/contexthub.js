window.ContextHubKernelConfig={debug:false,initializationTimeout:2000,stores:{geolocation:{type:"contexthub.geolocation",required:true,config:{geocoder:{enabled:true}}},surferinfo:{type:"contexthub.surferinfo",required:true},profile:{type:"granite.profile",required:true},emulators:{type:"granite.emulators",required:true},eventdata:{type:"aem.analyticsdata",required:true},pagedata:{type:"aem.pagedata",required:true},segmentation:{type:"aem.segmentation",required:true},abandonedproducts:{type:"commerce.abandonedproducts",required:true},recentlyviewed:{type:"commerce.recentlyviewed",required:true},cart:{type:"commerce.cart",required:true},orderhistory:{type:"commerce.orderhistory",required:true},relatedproducts:{type:"commerce.relatedproducts",required:true},smartlists:{type:"commerce.smartlists",required:true},seeddata:{type:"campaign.seeddata",required:false},metadata:{type:"campaign.metadata",required:false}}};
/*!
...
this.setItem("latitude",parseInt(e.latitude*1000000,10)/1000000);
this.setItem("longitude",parseInt(e.longitude*1000000,10)/1000000)
};
...
(function(f,e){var a={service:{jsonp:false,timeout:1000,ttl:30*60*1000,secure:e.location.protocol==="https:",host:"maps.googleapis.com",port:80,path:"/maps/api/geocode/json"},geocoder:{enabled:false},eventDeferring:16,html5coordinatesDiscoveryAPI:{timeout:30000,ttl:15*60*1000,highAccuracy:false},signatureGenerator:ContextHub.Paths.CONTEXTHUB_PATH+"/geolocation.signature.json",initialValues:{defaultLocation:{latitude:37.331375,longitude:-121.893992,address:{country:"United States",countryCode:"US",city:"San Jose",street:"Almaden Blvd",streetNumber:"151",region:"CA",name:"151 Almaden Blvd, San Jose, CA 95113, USA",postalCode:"95113"},addressDetailsOf:{latitude:37.331375,longitude:-121.893992}}}};
...
}ContextHub.eventing.on("ui-container-opened",function(){this.queryService(false,true)
...
if(e.CQ_Analytics&&CQ_Analytics.ClientContextMgr){var h=function(){var m=CQ_Analytics.ClientContextMgr.getRegisteredStore("geolocation");
...
c.prototype.generateSignature=function(n,l){if(!this.useGeocoder){return
}if(!ContextHub.UI||!ContextHub.UI.Container.isExpanded()){return
}var k=this.getItem("addressDetailsOf")||{};
var m=this.getItem("latitude");
var i=this.getItem("longitude");
if((k.latitude===m)&&(k.longitude===i)){return
}var g=this.config.initialValues.defaultLocation;
if(g&&(m===g.latitude)&&(i===g.longitude)){this.setItem("address",g.address);
this.setItem("addressDetailsOf",g.addressDetailsOf);
return
...
var j=this.getServiceURL(true);
j=j.replace(/^https?:\/\/[^/]*/,"");
var h=this;
f.ajax({url:this.config.signatureGenerator,method:"get",dataType:"json",cache:false,data:{url:j}}).done(function(o,r,q){if(q&&q.responseJSON){var p=h.config.service.params;
p.client=q.responseJSON.client||"";
p.channel=q.responseJSON.channel||"";
p.signature=q.responseJSON.signature||"";
n()
...
this.setItem("addressDetailsOf",{latitude:this.getItem("latitude"),longitude:this.getItem("longitude")})
}return l
};
...
this.setItem("latitude",parseInt(g.latitude*1000000,10)/1000000);
this.setItem("longitude",parseInt(g.longitude*1000000,10)/1000000)
};
...
}else{f.when(this.getCoordinates()).then(function(j){g.storeLocation(j.coords||{},g.config.initialValues.defaultLocation);
g.generateSignature(function(){g.uber("queryService",h)
})
},function(j){g.failureHandler(j);
g.storeLocation({},g.config.initialValues.defaultLocation);
g.generateSignature(function(){g.uber("queryService",h)
})
})
}};
...
ContextHub.console.log(ContextHub.Shared.timestamp(),"[loading] contexthub.store.commerce.smartlists - store.smartlists.js");
(function(c){var a={service:{jsonp:false,timeout:1000,ttl:30*60*1000,path:"${variable:ContextHub.Paths.RESOURCE_PATH}.commerce.smartlists.json"}};
var b=function(e,d){this.config=c.extend(true,{},a,d);
this.init(e,this.config);
this.profileLoaded=false;
var f=this;
...
b.prototype.products=function(h){var e=this.getTree();
var f=null;
var d=[];
if(e){if(h){for(var g=0;
g<e.length;
g++){if(e[g].path===h){f=e[g].entries;
break
}}}else{if(e[0]){f=e[0].entries
}}}for(g=0;
(f&&g<f.length);
g++){d.push(f[g])
}return d
};
ContextHub.Utils.storeCandidates.registerStoreCandidate(b,"commerce.smartlists",0)
}(ContextHubJQ));
...
},getFilteredTree:function(h){function i(m){for(var k in m){if(!m.hasOwnProperty(k)){continue
}if(m[k].type&&!h(m[k])){delete m[k]
}else{if(m[k].content){m[k].content=i(m[k].content);
var l=true;
for(var j in m[k].content){if(m[k].content.hasOwnProperty(j)){l=false;
break
}}if(l){delete m[k]
}}}}return m
}var g=d.extend(true,{},this.getTree());
return i(g)
